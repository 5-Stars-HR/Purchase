...
const product = (num) => {
  ...
  for (let i = 0; i < num; i++) {
    ...
    seed += `INSERT INTO products (productName, price, reviewCount, rating, themeName, themeUrl, featured, chokingHazard, productLimit, productUrl, onlineAvailability) VALUE ("${productName}", "${price}", "${reviewCount}", "${rating}", "${themeName}", "${themeUrl}", "${featured}", "${chokingHazard}", "${productLimit}", "${productUrl}", "${onlineAvailability}"); \n`
    ...
    if ((i + 1) % 1000 === 0) console.log(`${i} entries written`)
  }
}
...
const stores = (num) => {
  for (let i = 0; i < num; i++) {
    ...
    let query = `"${storeName}" "${street}" "${city}" "${state}" "${zip}"`
  }
}
...
const filePath = path.join(__dirname, './seeder/seed.txt')
fs.writeFile(filePath, seed, (err) => {
  if (err) {
    console.log('schema write fail')
  } else {
    console.log('prost');
  }
})
